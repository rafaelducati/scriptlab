...
//= require ../global/plugins/inputmask
//= require ../global/plugins/jquery.inputmask
...
// muda o form de acordo com o radio selecionado
if ( $(".perfilCadastro").length > 0 ) {
	$("input[type=radio][name='customer.type']").change(function() {

        $(".input-cpf-or-cnpj").addClass('hidden');
        $(".input-document").removeClass('hidden');

		if ($(this).val() == "B2C") {
			$(".input-label-document").html("CPF*");
			$(".panel-pj").addClass('hidden');
			$(".shopName").addClass('hidden');
			$("#stateRegistration").parent().addClass('hidden');
			$("#document").removeAttr('disabled');
			$("#document").inputmask({ "mask": "999.999.999-99" });


		} else if ($(this).val() == "MECHANIC" || $(this).val() == "OTHERS") {

            $(".input-cpf-or-cnpj").removeClass('hidden');
            $(".input-document").addClass('hidden');
            $("input[type=radio][name='customer.typeAux']").attr('checked',false);
			...
			$(".input-label-document").html("CNPJ*");
			$(".panel-pj").removeClass('hidden');
			$(".shopName").addClass('hidden');
			$("#stateRegistration").parent().removeClass('hidden');
			$("#document").removeAttr('disabled');
			$("#document").inputmask({ "mask": "99.999.999/9999-99" });
		}
	});


    $("input[type=radio][name='customer.typeAux']").change(function() {
...
// Muda máscara conforme valor digital no campo de CPF/CPNJ quando for "MECHANIC" ou "OTHERS"
// comentado para nao precisar mais validar desta forma para esses dois campos
/*
if ( $("#document").length > 0 ) {
	$("#document").on('focusout', function() {
		var radio = $("input[name='customer.type']:checked").val();
		var userType = $(this).val().replace(/[^\d]+/g,'');

		if ( radio == "MECHANIC" || radio == "OTHERS" ) {

			if ( userType.length > 11 ){
...
$(function() {
	var $radios = $("input:radio[name='customer.type']");

    if($radios.is(":checked") === false) {
		$radios.filter("[value=" + $("#customer-type").attr("datatype") + "]").click();
	}

    var $radios2 = $("input:radio[name='customer.typeAux']");
    if($radios2.is(":checked") === false) {
        $radios2.filter("[value=" + $("#customer-typeAux").attr("datatype") + "]").click();
    }
});
