...
ySearchTabs = function() {
	$('.y-tab').click(function() {
	    $('.y-tab').removeClass('y-active');
	    $(this).addClass('y-active');
	    var yActivate = $(this).attr('id');
	    $('.y-selectVehicle .y-ctr').removeClass('y-active');
	    $('.y-selectVehicle .y-ctr.' + yActivate).addClass('y-active');
	});
};
...
var yFired = 0;
yFilteredSelects = function() {
	if (yFired == 0) {
		$('#y-vehicleSel').val() ? yVehicleSel.val($('#y-vehicleSel').val()).trigger("change") : null;
		$('#y-makeSel').val() ? yMakeSel.val($('#y-makeSel').val()).trigger("change") : null;
		$('#y-modelSel').val() ? yModelSel.val($('#y-modelSel').val()).trigger("change") : null;
		$('#y-yearSel').val() ? yYearSel.val($('#y-yearSel').val()).trigger("change") : null;
		yFired = 1;
	}
};

yBannerMargin = function() {
	if ($('.y-headingsWrap .mktplace-banner .bannerList').length) {
		$('.y-headingsWrap .mktplace-banner').addClass('y-negativeMargin');
	};
};
...
//= require ../global/plugins/y-maskLib.js

yScrollToStep = function() {
    if (window.location.pathname.indexOf('servicos') > -1) {
        if (window.location.pathname.indexOf('veiculo') >= 0 && window.location.search.indexOf('ano') == -1) {
            $("html, body").animate({ scrollTop: $('.y-vehicleWrap').offset().top - $(window).height() / 4 }, 1000);
        } else if (window.location.search.indexOf('ano') >= 0) {
            $("html, body").animate({ scrollTop: $('.y-servicesWrap').offset().top - $(window).height() / 4 }, 1000);
        };
    };
};
...
yCep = function() {
    if (window.location.pathname.indexOf('servicos') > -1) {
        $('.y-zipInput').mask('00000-000');

        $('.y-zipInput').keyup(function() {
            if ($(this).val().length == 9) {
                yCep = $(this).val().replace('-', '');
                window.location.pathname = window.location.pathname.indexOf('cep') == -1 ? window.location.pathname + '/cep-' + yCep : '/servicos/cep-' + yCep;
            }
        });

        $('.y-zipInput').focus(function() {
            $('.y-minicartWrapper').addClass('y-keyboard');
        });

        $('.y-zipInput').blur(function() {
            $('.y-minicartWrapper').removeClass('y-keyboard');
        });
    }
};

yServiceFilters = function() {
    $('.y-filter').click(function() {

        $('.y-filter').removeClass('y-active');
        $(this).toggleClass('y-active');

        filterText = $(this).attr('data-filter');
        $yMatchs = []
        $('.y-serviceBox').each(function() {
            serviceText = $(this).find('.y-title .y-productName').text();
            if (serviceText == filterText) {
                $yMatchs.push($(this));
            }
        });

        $('.y-serviceBox').hide();
        $.each($yMatchs, function(i, y) {
            $(y).show();
        });

    });
    $('.y-removeFilter').click(function() {
        $('.y-serviceBox').show();
    });
};

yServiceBuy = function() {
    $(".y-serviceBox").click(function() {
        var stockId = $(this).data('stockid');
        $.post("/p/adicionar-servico", {
                id: stockId,
                quantity: 1
            },
    ...
    $(".y-serviceBox .y-additionalInfo").click(function(e) {
        e.stopPropagation();
    });
};

yServiceRemove = function() {
    $('body').on('click', '.y-removeBt', function() {
        var reserveId = $(this).data('reserve-id');
        $.post("/carrinho-remover-servico/" + reserveId,
        function(data, status) {
            yGetCartJson();
        });
    });
...
yGetCartJson = function() {
    $.getJSON('/carrinho-servicos-json', function(data) {
        $('.y-minicart .y-totalValue').text(data.total);
        $('.y-minicart .y-totalItems').text(data.quantity + (data.quantity <= 1 && data.quantity != 0 ? ' serviço' : ' serviços'));
        $('.y-minicart .y-itens').html('');
        data.quantity > 0 ? $('.y-itensT').show() : $('.y-itensT').hide();
        $.each(data.reserves, function(i, y) {
            $('.y-minicart .y-itens').append('\
				<div class="y-item">\
                    <span class="y-serviceName">' + y.quantity + 'x</span>\
                    <span class="y-serviceName">' + y.name + '</span>\
                    <span class="y-separator">|</span>\
                    <span class="y-servicePrice">' + y.total + '</span>\
                    <span class="y-removeBt" data-reserve-id="' + y.id + '">Remover</span>\
                </div>\
			');
        });
        yBuyBt();
    });
    $.get("/carrinho/quantidade", function(data) {
...
yGetPrices = function() {
    $('.y-serviceBox .y-price').each(function() {
        var self = $(this);
...
yGetAddress = function() {
    if ($('.y-addressC').length) {
    	yCEP = $('.y-addressC').attr('data-cep');
    	$.get("/buscar-endereco/"+yCEP, function(data) {
    		$('.y-addressC').get(0).lastChild.nodeValue = yCEP + ' - ' + data.city + ' - ' + data.state;
    	});
    };
};

yDetailsModal = function() {
	$('.y-catInfo').click(function() {
        $(this).next('.y-modalWrap').show();
	});

	$('.y-modalWrap, .y-closeBt').click(function() {
		$('.y-modalWrap').hide();
	});

	$('.y-addInfoBox').click(function(e) {
        e.stopPropagation();
    });
};
...
yFixedCart = function() {
    ...
    if ($('.y-minicart').length) {
        stickyMapTop = $('.y-minicart').offset().top - 20;
        stickyMapBottom = ($('.y-steps').offset().top + $('.y-steps').outerHeight()) - ($('.y-minicart').outerHeight(true));
        directoryBottom = stickyMapBottom - stickyMapTop + (parseInt($('.y-minicart').css("marginTop").replace('px', '')));
        yOffsetLeft = $('.y-minicart').offset().left;
        yWidth = $('.y-minicart').width();
        $(window).scroll(function() {
            var viewportWidth = $(window).width();
            if (viewportWidth > 1024) {
                if ($(window).scrollTop() > stickyMapTop && $(window).scrollTop() < stickyMapBottom) {
                    $('.y-minicart').css({ position: 'fixed', top: '20px', left: yOffsetLeft, width: yWidth, margin: '0px' });
                } else {
                    //original
                    //$('.y-minicart').css({position: 'static', top: '0px'});
                    if ($(window).scrollTop() >= stickyMapBottom) {
                        //secondary position check to see what new positioning style to apply?
                        $('.y-minicart').css({ position: 'relative', top: directoryBottom, left: '0px'});
                    } else if ($(window).scrollTop() < stickyMapTop) {
                        //needed?
                        $('.y-minicart').css({ position: 'relative', top: '0px', left: '0px'});
                    }
                }
            } else {
                $('.y-minicart').css({ position: 'relative', top: '0px' });
            }
        });
    }
};
...
yBuyBt = function() {
    if ($('.y-serviceWrapper:not(.y-payment) .y-itens .y-item').length) {
        $('.y-buyWrap').show();
    } else {
        $('.y-serviceWrapper:not(.y-payment) .y-buyWrap').hide();
    };
};
...
//= require ../global/plugins/inputmask.js
//= require ../global/plugins/sweetalert2.js
//= require ../global/plugins/jquery.inputmask.js
//= require y-search.js
//= require y-vehicle.js

yHomeSlider = function() {
	if ($(".y-slider .bannerList .bannerItem").length > 1) {
		$(".y-slider").cycle({
			slides: '.bannerList .bannerItem'
		});
		$(".y-sliderArrow").show();
	}
};
...
yCTCmask = function() {
    $('.y-clickToCall #cliente_telefone').inputmask({"mask": "(99) 999999999"});
};
...
//= require ../global/vendor/owl.carousel.min.js
...
//= require ../global/vendor/jquery
//= require custom-bootstrap
//= require ../global/plugins/jquery.autocomplete.min
//= require ../global/plugins/jquery.highlight
//= require ../global/plugins/flickity.pkgd.min.js
//= require ../global/plugins/jquery.cookie.min.js
//= require ../global/plugins/ouibounce.min.js
//= require ../global/plugins/jquery.cycle2.min.js
//= require y-scripts.js
//= require ../global/vendor/magnify
//= require productShelf.js
//= require_self

Number.prototype.toReais=function() {
	var value = this;
	var yFormatted = value.toFixed(2).replace('.',',').replace(/d(?=(d{3})+,)/g, '$&.')
    return 'R$ ' + yFormatted;
};

function saveVehicle(params) {
    var garageSuccess = $("#garage-alert-success");
    var garageError = $("#garage-alert-danger");
	var array = params.replace(/ /g,'').split(',')
	equipmentType = array[0];
	make = array[1];
	model = array[2];
...
$(document).ready(function() {
	...
	$('.y-searchTerm').click(function() {
		var yTerm = $(this).attr('data-search');
		$('#ySearch').find('input[name=q]').val(yTerm)
		$('#ySearch').submit();
	});


	/* ==========================================================================
	 Link para remover os veículos
	 ========================================================================== */
	var url = window.location.href;

	url = url.replace(/(http:|https:)\/\/(www\.)?(\w|\d)+.+?\//, "");

	url = url.replace(/(\?|&)(veiculo)=([^&]+)/, "");
	url = url.replace(/(\?|&)(montadora)=([^&]+)/, "");
	url = url.replace(/(\?|&)(modelo)=([^&]+)/, "");

	url = url.replace(/(carro|caminhao|onibus|trator|moto)\/.*\/(categoria)-/, "categoria/");
	url = url.replace(/(carro|caminhao|onibus|trator|moto)\/.*\/(loja)-/, "loja/");
	url = url.replace(/(carro|caminhao|onibus|trator|moto)\/.*\/(marca)-/, "marca/");
	url = url.replace(/(carro|caminhao|onibus|trator|moto)\/.*\?/, "?");
	url = url.replace(/(carro|caminhao|onibus|trator|moto)\/.*/, "");

	url = url.replace(/\/(veiculo-.*)\/categoria/, "/categoria");
	url = url.replace(/\/(veiculo-.*)\/loja/, "/loja");
	url = url.replace(/\/(veiculo-.*)\/marca/, "/marca");
	url = url.replace(/\/(veiculo-.*)(\?)/, "?");
	url = url.replace(/\/(veiculo-.*)(\/)/, "/");
	url = url.replace(/\/(veiculo-.*)/, "");
	...
	$(".clean-vehicle").attr("href", '/?remover=sim');

	/* ==========================================================================
	Setup para os banners do mktplace
	========================================================================== */
	$('.bannerList').each(function(){
		if( $(this).find('.bannerItem').size() > 1 ){
		}
	})
	...
	/* ==========================================================================
	Pega quantidade para adicionar ao carrinho.
	========================================================================== */
	updateHeaderCart();

	/* ==========================================================================
	Abre modal que utiliza URL
	========================================================================== */
	$(".url-modal").on('click', function(e) {
		e.preventDefault();
		var url = $(this).attr('data-content');
		var title = $(this).attr('title');
		$('.modal-body-cdp').load(url, function(result){
			$('.inside-title').hide();
			$(".modal-title").html(title);
			$('.modal-footer-cdp').modal("show");
		});
	});

	/* ==========================================================================
	Limpa conteúdo do modal de badge
	========================================================================== */
	if($('#badge-info-modal')) {
		$('#badge-info-modal').on('hidden.bs.modal', function () {
	...
	Busca por escopo
	==========================================================================
	var $searchDelimiter = $('.searchDelimiter'),
		$searchInput = $('.form-control'),
		$searchDelimiterCheck = $searchDelimiter.find('input[type="checkbox"]');
	if( $searchDelimiter.size() > 0 ){
		var $inputPadding = $searchDelimiter.outerWidth() + 5;
		$searchInput.css({
			'padding-right' : $inputPadding
		})
		$searchDelimiterCheck.on('change', function(){
			$searchDelimiter.toggleClass('checked');
			if( $(this).is(':checked') ){
				$(this).next('.glyphicon').removeClass('glyphicon-check').addClass('glyphicon-unchecked')
			}else{
				$(this).next('.glyphicon').removeClass('glyphicon-unchecked').addClass('glyphicon-check')
			}
		})
	}
	*/
	var text = $('#searchType option:selected').text();
	var larg = $("#width_tmp").html(text).width();
	var total = larg+15;

	if ( total > 120 ) {
		$("#searchSelect").addClass('selectWidth');
		$("#q").attr('placeholder', "Pesquise dentro da opção selecionada");
		$(".selectWidth").css("width", total);
	}

	/* ==========================================================================
	Troca o placeholder da barra de busca
	========================================================================== */
	$("#searchType").on('change', function(event) {
		event.preventDefault();

		if ( $(this).val() == "produto" ){
			$("#q").attr('placeholder', "Pesquise aqui. Ex.: 'Amortecedor para gol 2010' ou 'Disco de freio Palio 2012'");
			$("#searchSelect").removeClass('selectWidth');
			$("#searchSelect").removeAttr('style');
		} else if ( $(this).val() == "loja" ) {
			$("#q").attr('placeholder', "Pesquise aqui o nome da loja desejada. Ex.: 'Loja do Bruno' ou 'Mercado Autopeças'");
			$("#searchSelect").removeClass('selectWidth');
			$("#searchSelect").removeAttr('style');
		} else if ( $(this).val() == "marca" ) {
			$("#q").attr('placeholder', "Pesquise aqui uma marca. Ex.: 'Cofap' ou 'Bosch'");
			$("#searchSelect").removeClass('selectWidth');
			$("#searchSelect").removeAttr('style');
		} else {
			$("#q").attr('placeholder', "Pesquise dentro da opção selecionada");
			if ( total > 120 ) {
				$("#searchSelect").addClass('selectWidth');
				$("#searchSelect").css("width", total);
			}
		}
	});

	/* ==========================================================================
	 Highlight no resultado de busca
	 ========================================================================== */
 	if ($(".searchResult").length > 0) {
 		if ( $("#q").val() != ""){
 			/* Primeiro crio a função para tirar palavras de junção e outros irrelevantes */
 			String.prototype.removeStopWords = function() {
				...
				for(x=0; x < words.length; x++) {
					word = words[x].replace(/\s+|[^a-z\w\s\/@-]+/ig, "");
			...
			/* aqui pego os termos na barra de busca e separo em uma lista */
			var searchResultTerm = $("#q").val().removeStopWords().split(" ");

			var regex_sing = new RegExp(/([e]+[s]\b)|(s\b)/g);

			/* pra cada item dessa lista, removo o plural */
			/* no fim, depois de 0.5s, aplico o highlight */
			setTimeout( function() {
				for ( i = 0; i < searchResultTerm.length; i++ ) {
					if ( searchResultTerm[i].length > 3 ) {
						searchResultTerm[i] = searchResultTerm[i].replace(regex_sing, "");
					}
					if ( searchResultTerm[i].length > 1 ) {
						$('#showcase .ellipsis_text').highlight(searchResultTerm[i], true);
						$('#showcase .title').highlight(searchResultTerm[i], true);
					}
				}
			}, 500);
 		}
	...
	/*
	for (i = 0; i < searchResultTerm.length; i++) {
		$('#showcase').highlight(searchResultTerm[i], true);
	};
	*/

	/* roda autocomplete no document.ready */
	if ( $("#q").length > 0 ) {
		$("#searchType").on('change', function(event) {
			event.preventDefault();
			if ( $("#searchType").val() == "produto" ) {
				autocomplete();
			} else {
	...
	/* ==========================================================================
	 Função do clique: navegação no footer
	 ========================================================================== */
    $(".footerMenu h4").bind("click", function(){
...
function search(event) {
	...
	if ($('#searchType').val() == 'local') {
		...
		var value = $('#q').val();
		var url = updateQueryString('pagina', undefined);
		var url = updateQueryString('orcamento', undefined, url);
		window.location.href = updateQueryString('q', value, url);
	} else {

        // salvar keyword antes de fazer a busca
        var mainSearchForm = $('#mainSearch');
        mainSearchForm.submit();
	}
}

function ySearch(event) {
    event.preventDefault();
    event.stopPropagation();
    var formQ = $('#ySearch');
    formQ.submit();
		...
		if (navigator.geolocation) {
			try {
				...
				navigator.geolocation.getCurrentPosition(function(position) {
					...
					$.ajax({
						type: "POST",
						url: "/atualizar-localizacao",
						data: { latitude: position.coords.latitude, longitude: position.coords.longitude }
					});
				});

				setCookie('geo', true, date, '/', true);
...
function setCookie(name, value, date, path, secure) {
    if (enviroment == 'development') {
        $.cookie(name, value, {expires: date, path: path});
    } else {
        $.cookie(name, value, {expires: date, path: path, secure: secure});
    }
}


function yOpenSaveCartForm() {
    $('.y-openForm').hide();
    $('.y-form').show();
}

function fetchSkusPrices(){

    var prices = $.map($('[data-template]'), function (element) {
        var self = $(element);
        var skuLink = self.data('sku-link');
        var seller = self.data('seller');
        ...
        return $.ajax({
            ...
            dataType: 'json',
            success: function (data) {
                self.html(window.cdpTemplates.apply(templateName, {sku: data, skuLink: skuLink, seller: seller}));
            }
        });
    });

    var adjustItemsHeight = function () {
        var items = $("#showcase .grid .item-container");
